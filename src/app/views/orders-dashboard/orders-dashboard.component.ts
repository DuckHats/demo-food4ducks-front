...
export class OrdersDashboardComponent implements OnInit, OnDestroy {
  ...
  UILabels = UILabels;
  activeTab: string = UILabels.ADMIN.DASHBOARD.TABS.ORDERS;
  ...
  loadAllData(): void {
    if (this.activeTab === UILabels.ADMIN.DASHBOARD.TABS.MENUS) {
      this.loadMenusWeek();
    } else if (this.activeTab === UILabels.ADMIN.DASHBOARD.TABS.ORDERS) {
      this.loadOrders(this.formatDate(this.selectedDate));
    } else if (this.activeTab === UILabels.ADMIN.DASHBOARD.TABS.USERS) {
      this.loadUsers();
    }
  }
