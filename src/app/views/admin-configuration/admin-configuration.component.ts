...
export class AdminConfigurationComponent implements OnInit {
  ...
  deleteImage(id: number): void {
    if (confirm(this.UILabels.ADMIN.CONFIG.IMAGE_DELETE_CONFIRM)) {
  ...
  updateImageDates(img: any): void {
    if (!img.start_date || !img.end_date) {
      this.alertService.show(
        'error',
        this.UILabels.ADMIN.CONFIG.DATES_REQUIRED,
        ''
      );
      return;
    }
    ...
    this.imageService
      ...
      .subscribe({
        next: (res) => {
          this.alertService.show(
            'success',
            this.UILabels.ADMIN.CONFIG.DATES_UPDATE_SUCCESS,
            ''
          );
          this.loadImages();
        },
        error: (err) => {
          this.alertService.show(
            'error',
            this.UILabels.ADMIN.CONFIG.DATES_UPDATE_ERROR,
            ''
          );
        },
      });
